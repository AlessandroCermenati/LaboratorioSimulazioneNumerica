...
int main (int argc, char *argv[]){
   ...
   if (input.is_open()){
    ...
    for(int i=0; i< blocks; i++){
        sum=0;
        for(int j=0; j<L; j++){
            move[0] = origin[0] + (rnd.Gauss(0. , 0.75));        //calcola la mossa x' con prob. di trans. uniforme su un intervallo di più o meno un'unità di raggio di Bohr
            move[1] = origin[1] + (rnd.Gauss(0. , 0.75));
            move[2] = origin[2] + (rnd.Gauss(0. , 0.75));
    ...
    for(int i=0; i< blocks; i++){
        sum=0;
        for(int j=0; j<L; j++){
            move[0] = origin[0] + (rnd.Gauss(0. , 1.85));
            move[1] = origin[1] + (rnd.Gauss(0. , 1.85));
            move[2] = origin[2] + (rnd.Gauss(0. , 1.85));
