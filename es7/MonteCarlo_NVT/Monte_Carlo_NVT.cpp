...
//update of the histogram of g(r)
	...
	if(dr>(double)(k-igofr)*lbin && dr<=(double)(k-igofr+1)*lbin){
		walker[k] = walker[k] +2.0;
	}
     }
...
void Averages(int iblk) //Print results for current block
{
   ...
   const int wd=12;
    ...
    for(int k = igofr; k<igofr+nbins; k++){
        double norma = (4.0/3.0)*M_PI*(pow((double)(k-igofr+1)*lbin,3) - pow((double)(k-igofr)*lbin,3))*rho*npart;
        gdir = blk_av[k]/norma/blk_norm;
        glob_av[k] += gdir;
...
//g(r)
    Gofr << iblk;
    for(int k=igofr; k<igofr+nbins; k++){
        Gofr << setw(wd) << glob_av[k];
    }
    Gofr<<endl;

    if(iblk == nblk){
        for(int k=igofr; k<igofr+nbins; k++){
            double errg = Error(glob_av[k],glob_av2[k],iblk);
            Gave << (double)(k-igofr)*lbin + lbin/2.0 << setw(wd) << glob_av[k]/(double)iblk << setw(wd) << errg << endl;
        }
	}
