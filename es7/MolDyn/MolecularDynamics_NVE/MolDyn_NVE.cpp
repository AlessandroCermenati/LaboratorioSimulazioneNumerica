...
int main(){
    ...
    meanG.open("ave_radial.dat");
    sigmaG.open("err_radial.dat");
...
//        ConfXYZ(nconf);//Write actual configuration in XYZ format //Commented to avoid "filesystem full"! 
     ...
     if(istep%(10*L) == 0){                       //L è il numero di misure per blocco; poichè si ha una misura ogni 10 step, il blocco si conclude ogni 10*L step
		...
		AV2P[j]=pow(AVP[j],2);
        for(int g = 0; g<100; g++){
            AVG[g][j] = sumradial[g]/(double)L;
            AV2G[g][j] = pow(AVG[g][j],2);
            sumradial[g] = 0;
		}
  ...
  for(int i=0; i<nblocks; i++){                               //avendo salvato le medie a blocchi nei vettori AV, il calcolo del data blocking si può fare in seguito;
		for(int j=0; j<i+1; j++){
            ...
            for(int g = 0; g < 100; g++){
                progG[g][i] = progG[g][i] + AVG[g][j];
                prog2G[g][i] = prog2G[g][i] + AV2G[g][j];
			}
		}
		progT[i] = progT[i]/(i+1);
        ...
        for(int g = 0; g<100; g++){
            progG[g][i] = progG[g][i]/(i+1);
            prog2G[g][i] = prog2G[g][i]/(i+1);
		}
		if(i==0){
			errT[i] = 0;
            errK[i] = 0;
            ...
            for(int g=0; g<100; g++){
                errG[g][i] = sqrt((prog2G[g][i] - pow(progG[g][i],2))/i);
		    }
		}
	}
    ...
    for(int g=0; g<100; g++){
     meanG<<progG[g][nblocks-1]<<endl;
     sigmaG<<errG[g][nblocks-1]<<endl;
	}
...
void Input(void){ //Prepare all stuff for the simulation
  ...
  binl = box/200.0;
  cout << "Lunghezza del bin dell'instogramma della g(r) = " << binl << endl;
...
//cycle over pairs of particles
  for (int i=0; i<npart-1; ++i){
    for (int j=i+1; j<npart; ++j){
     ...
     for(int k = 0; k < 100; k++){                                      //calcolo della radial distribution function
        if(binl*(double)k < dr && binl*(double)(k+1) >= dr){
            radialf[k] = radialf[k] + 2.0;  
		}
	 }
    }          
  }
  ...
  for(int k=0; k<100; k++){
      double norma = (4.0/3.0)*M_PI*rho*npart*(pow(binl*(double)(k+1),3) - pow(binl*(double)k,3));
      radialf[k] = radialf[k]/norma;
  }
