...
double variationalWaveFunction(double v)
{
	...
	//return exp(-0.5*v*v);
	double mu_op = 0.823;
	double sigma_op = 0.6455;
	double A = exp(-((pow(v-mu_op,2))/(2*sigma_op*sigma_op)));
	double B = exp(-((pow(v+mu_op,2))/(2*sigma_op*sigma_op)));
	return A + B; 
}
...
double variationalWaveFunction_second(double v)
{
	...
	double A = exp(-((pow(v-mu_op,2))/(2*sigma_op*sigma_op)));
    	double B = exp(-((pow(v+mu_op,2))/(2*sigma_op*sigma_op)));
    	return pow((v-mu_op)/(sigma_op*sigma_op),2)*A + pow((v+mu_op)/(sigma_op*sigma_op),2)*B - (A+B)/(sigma_op*sigma_op);
}
